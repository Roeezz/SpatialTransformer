...
if __name__ == '__main__':
    ...
    for filename in os.listdir(data_folder):
        if filename.endswith('.npy'):
            ...
            confidence = myUtiles.Get_compare_video(video, bboxs)
            video = video.permute(1, 2, 3, 0)
            bboxs = bboxs.permute(1, 2, 3, 0)
            confidence = torch.argsort(confidence, dim=1)
            for img_num, (img, bbox, conf) in enumerate(zip(video, bboxs, confidence)):
                ...
                while x == 'n':
                    cv2.imshow('label', cv2.resize(labels[conf[guss_index]], (480, 480)))
                    cv2.imshow('image', cv2.resize(cv2.cvtColor(np.float32(img), cv2.COLOR_RGB2BGR), (480, 480)))
                    cv2.waitKey(1)
